...
- **CLI での「返答の未完了」**。CLI（`entrypoint: cli`）では、`Stop` フックが走る時点で
  直前の assistant レコードにまだ `stop_reason` が書かれていないことがあるため、この軸だけ
  自動的に無効化します。保存が終わった `.jsonl` を後から数えると CLI でも欠けは 0 件
  （2.1.267〜268 の 1,563 件）だったので、これは記録の欠落ではなくフック実行時点のタイミングの問題です。
